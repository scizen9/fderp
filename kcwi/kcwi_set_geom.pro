; $Id: kcwi_set_geom.pro | Tue Apr 21 10:32:09 2015 -0700 | Don Neill  $
;
...
pro kcwi_set_geom,kgeom,ikcfg,ppar, help=help
	...
	kgeom.ccoff = fltarr(12)
	;
	; check for CWI data
    	if strtrim(strupcase(kcfg.instrume),2) eq 'CWI' or $
	   strtrim(strupcase(kcfg.instrume),2) eq 'FCWI' or $
	   strtrim(strupcase(kcfg.instrume),2) eq 'PCWI' then begin
		;
		...
		if strtrim(kcfg.gratid,2) eq 'RED' then begin
			...
			kgeom.ccwn = 40./kgeom.ybinsize	; Pixels
			kgeom.rho = 1.20d
			kgeom.slant = 0.0d
			kgeom.lastdegree = 5
			;
